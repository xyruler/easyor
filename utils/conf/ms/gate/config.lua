...
_M.caches = {
	senssion = {
		share_dict_size = '1m',		--共享内存大小
		lru_size = 100000,			--worker中缓存的最大对象个数
		ttl = 86400,				--缓存有效时间，单位秒
	...
	account = {
		...
		neg_ttl = 60,				--缓存中未命中对象有效时间，单位秒
		database = {				--绑定数据库
			type = 'mysql',			--数据库类型，支持-mysql,redis,user
			master = 'account',		--读写库配置名，配置数据在_M.db中定义
			slave = 'account',		--只读库配置名，默认为master配置
			table = 't_account',	--表名
			keys = {'account'},		--唯一索引，可以是多个字段
			auto_incr = 'uid',		--自增字段名，插入新数据时自动生成
			value_keys = {'uid','account','password','regtime','status'},
		},
	},
}
